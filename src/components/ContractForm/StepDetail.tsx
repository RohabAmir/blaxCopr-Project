...
import UpDownIcon from "../../../public/icons/UpDown.svg";
import {
  useForm,
  FormProvider,
  useFieldArray,
...
const StepDetail: FC<any> = ({ handleStepChange, step }) => {
  ...
  useEffect(() => {
    if (isSuccess && contractDetails) {
      ...
      methods.reset({
        ...
        escrowFeePaidBy: contractDetails.shipping?.escrowFeePaidBy,
  ...
  const onSubmit = async (data: any) => {
    ...
    const storedTransactionIdsString = getLocalData("transaction_ids") || "[]";
    ...
    if ((transactionIds.length > 0 || shippingId) && isDirty) {
      ...
      try {
        ...
        if (
          response.data &&
          response.data.response.updatedContract.shipping.id &&
          transactionIds
        ) {
  ...
  return (
    <form className={styles.detailsMain} onSubmit={handleSubmit(onSubmit)}>
          ...
          >
            ...
            <Row className="w-full">
              ...
              <Controller
                ...
                // }
                render={({ field }) => (
                  <textarea {...field} rows={6} className={styles.textArea} />
                )}
              />
            </Row>
          </FormSection>
          ...
          <div className={styles.flex}>
            ...
            <div className={styles.summary}>
              <div className={styles.flexDetails}>
                ...
                <p className={styles.amount}>${subTotal.toFixed(2)}</p>
              </div>
              <div className={styles.flexDetails}>
              ...
              {!isMobile && (
                <div className={styles.flexDetailsLine}>
                  ...
                  <p className={styles.amount}>${escrowFee.toFixed(2)}</p>
                </div>
              )}
              {isMobile && (
                <>
              ...
              <div className={styles.flexDetails}>
                ...
                <p className={styles.amount}>${buyerPrice.toFixed(2)}</p>
              </div>
            </div>
          </div>
        </FormSection>
