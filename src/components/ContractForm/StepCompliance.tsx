...
const StepCompliance: FC<StepComplianceProps> = ({
  ...
  const { getRootProps, getInputProps } = useDropzone({
    onDrop: (acceptedFiles: File[]) => {
      ...
      const newFiles = acceptedFiles.reduce((acc, file) => {
        const isSupportedFormat = [".png", ".jpg", ".jpeg", ".pdf"].some(
  ...
  useEffect(() => {
    if (isSuccess && contractDetails && contractDetails.documents) {
      const formattedDocuments = contractDetails.documents.map((doc: any) => {
        // Or, if you want to extract just the filename from the URL:
        const documentName = extractFileName(doc.document);
  ...
  const onSubmit = async (data: any) => {
    ...
    const storedDocumentIdsString = getLocalData("document_ids") || "[]";
    ...
    if (documentIds.length > 0 && filesChanged) {
      ...
      try {
        const documentUrls = await uploadDocumentsToAzureAndGetUrls(
          uploadedFiles
        );
