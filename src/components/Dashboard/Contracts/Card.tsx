...
const Card: FC<ICard> = ({ data, userDetails, onDelete }) => {
  ...
  const handleClick = (e: { stopPropagation: () => void }) => {
    ...
    const transactionIds: TransactionId[] = data?.transactions?.map(
      (t: TransactionId) => ({ id: t.id })
    );
    storeLocalData("transaction_ids", JSON.stringify(transactionIds));
    const documentIds: documentId[] = data?.documents?.map((d: documentId) => ({
      id: d.id,
    }));
        ...
        >
          <div className={styles.flexContainerRow}>
            <div className={styles.flexRow}>
              ...
              {["APPROVE", "DELIVERED", "RECEIVED", "DISPUTE"].includes(
                data?.status
            ...
            >
              {isMouseOver && data?.status !== "APPROVE" ? (
                ...
                <>
                  ...
                  />
                  {status === "Closed" ? (
                    <span className={styles.close}>{roleType}</span>
                  ) : (
                    <span className={styles.span}>{roleType}</span>
                  )}
                </>
              )}
            </div>
