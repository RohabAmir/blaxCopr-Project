...
const AddWithDrawlDisbursement: FC<addWithDrawlProps> = ({
  ...
  const currencyFieldNames = formData.map((_, index) => `currency_${index}`);
  ...
  return (
    <>
      <form className={styles.sellerMain} onSubmit={handleSubmit(onSubmit)}>
        <FormProvider {...methods}>
          <div className={styles.withDrawlMain}>
            {formData.map((data, index) => {
              const currentCurrency = watchedCurrencies[index];
              return (
                <div key={data.id}>
                  {index === 0 ? (
                    <>
                      ...
                      <p className={styles.subHeadingWithdrawlLight}>
                        This account is designated for the disbursement of funds
                        by Blaxcorp upon the successful completion of a contract
                      </p>
                    </>
                  ) : (
